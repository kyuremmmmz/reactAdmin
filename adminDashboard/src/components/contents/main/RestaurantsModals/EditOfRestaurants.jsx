...
function EditOfRestaurants({ RestaurantsData, show, hide }) {
   const[festivalName, setFestivalName] = useState(null);
    ...
    const handleAmenityUrlChange = async (index, file) => {
        if (file) {
            const fileName = `${RestaurantsData.id}_${file.name}`;
...
export default EditOfRestaurants
